...
import ntk.android.biography.event.EvMessage;
...
public class ActRegister extends AppCompatActivity {
    ...
    public void ClickBtn() {
        if (((Button) findViewById(R.id.btnActRegister)).getText().equals("تایید شماره")) {
            if (Txt.getText().toString().isEmpty() || Date.isEmpty()) {
                Toast.makeText(this, "شماره موبایل و تاریخ تولد خود را وارد نمایید", Toast.LENGTH_SHORT).show();
            } else {
    ...
    @OnClick(R.id.btnDateActRegister)
    public void ClickSelectDate() {
        DatePickerDialog dialog = new DatePickerDialog();
        dialog.setThemeDark(false);
        dialog.show(getFragmentManager(), "انخاب تاریخ تولد");
        dialog.setOnDateSetListener((view, year, monthOfYear, dayOfMonth) -> {
            Date = year + "-" + (monthOfYear + 1) + "-" + dayOfMonth;
            ((Button) findViewById(R.id.btnDateActRegister)).setText(Date);
        });
    }
}
